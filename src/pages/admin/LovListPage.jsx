...
export default function LovListPage() {
      ...
      {!loading && !error && (
        <>
          <table className="w-full text-sm bg-white rounded-lg shadow-sm">
            ...
            <tbody>
              {pagedRows.map((row) => (
                <tr key={row.lov_id} className="border-t border-slate-100">
                  {COLUMNS.map((col) => (
                    <td key={col.key} className="px-4 py-2">{renderCell(row, col)}</td>
                  ))}
                  <td className="px-4 py-2 space-x-2">
                    <Link
                      to={`/admin/lov/${row.lov_id}`}
                    ...
                    <button
                      onClick={() => handleDelete(row.lov_id)}
