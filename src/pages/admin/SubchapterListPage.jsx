...
export default function SubchapterListPage() {
  ...
  // Reset to page 1 whenever the search term changes, so you don't get
  // stuck on a page number that no longer has any matching results
  useEffect(() => {
    setPage(1)
  }, [search])
      ...
      {!loading && !error && (
        <>
          <table className="w-full text-sm bg-white rounded-lg shadow-sm">
            ...
            <tbody>
              {pagedRows.map((row) => (
                <tr key={row.subchapter_id} className="border-t border-slate-100">
                  {COLUMNS.map((col) => (
                    <td key={col.key} className="px-4 py-2">{renderCell(row, col)}</td>
                  ))}
                  <td className="px-4 py-2 space-x-2">
                    <Link
                      to={`/admin/subchapter/${row.subchapter_id}`}
                    ...
                    <button
                      onClick={() => handleDelete(row.subchapter_id)}
